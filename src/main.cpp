...
// Diagnostic counters: raw BLE advertisement firehose, independent of
// Flock matching. Lets you tell "radio heard nothing" from "radio heard
// lots but nothing matched" after a field walk.
static volatile uint32_t fyTotalHeard = 0;
static uint32_t fyHeardAtLastReport = 0;
static unsigned long fyLastHeardReport = 0;
...
#ifdef PROMISC_LOG
#ifndef PROMISC_RSSI_MIN
#define PROMISC_RSSI_MIN -70
#endif
        if (rssi >= PROMISC_RSSI_MIN) {
            char mfrStr[8] = "----";
            if (dev->getManufacturerDataCount() > 0) {
                std::string d = dev->getManufacturerData(0);
                if (d.size() >= 2) {
                    snprintf(mfrStr, sizeof(mfrStr), "%02X%02X",
                             (uint8_t)d[1], (uint8_t)d[0]);
                }
            }
            printf("[PROMISC] %s mfr:0x%s rssi:%d name:\"%s\" [%s]\n",
                   addrStr.c_str(), mfrStr, rssi, name.c_str(),
...
<div class="sc"><div class="n" id="sH">0</div><div class="l">HEARD</div></div>
...
fetch('/api/stats').then(r=>r.json()).then(s=>{document.getElementById('sH').textContent=s.heard;let g=document.getElementById('sG');if(s.gps_valid){g.textContent=s.gps_tagged+'/'+s.total;g.style.color='#22c55e';}else{g.textContent='OFF';g.style.color='#ef4444';}}).catch(()=>{});}
...
// ============================================================================
// WIFI PROMISCUOUS SCAN MODE
// Detects Flock Safety camera WiFi APs and probe requests. Signatures from
// GainSec's Flock Safety research (2025): SSIDs "Flock-XXXXXX" (6 hex) and
// "test_flck" (dev SSID stored in production firmware), plus any SSID
// containing "flock" as a catch-all.
// ============================================================================
#include "esp_wifi.h"
#include <strings.h>

#ifndef WIFI_SCAN_RSSI_MIN
#define WIFI_SCAN_RSSI_MIN -100
#endif

static uint8_t fyWifiChannel = 1;
static unsigned long fyLastChannelHop = 0;
static uint32_t fyWifiMatchCount = 0;
static uint32_t fyWifiFramesSeen = 0;
static unsigned long fyLastWifiStats = 0;

// Dedup set for hidden-SSID BSSIDs so we alert once per unique AP per session.
// Linear-scan array (bounded) — fine for 64 entries in a tight callback.
#define FY_HIDDEN_DEDUP_SIZE 64
static uint8_t  fyHiddenBssids[FY_HIDDEN_DEDUP_SIZE][6];
static uint8_t  fyHiddenCount = 0;
static uint8_t  fyHiddenNextSlot = 0;

static bool fyWifiIsHiddenSSID(const uint8_t* data, uint8_t len) {
    if (len == 0) return true;           // SSID IE present but empty
    for (uint8_t i = 0; i < len; i++) {  // or padded with null bytes
...
static bool fyWifiSeenHidden(const uint8_t* mac) {
    uint8_t n = fyHiddenCount < FY_HIDDEN_DEDUP_SIZE ? fyHiddenCount : FY_HIDDEN_DEDUP_SIZE;
    for (uint8_t i = 0; i < n; i++) {
        if (memcmp(fyHiddenBssids[i], mac, 6) == 0) return true;
    }
    return false;
}

static void fyWifiAddHidden(const uint8_t* mac) {
    if (fyHiddenCount < FY_HIDDEN_DEDUP_SIZE) {
        memcpy(fyHiddenBssids[fyHiddenCount], mac, 6);
        fyHiddenCount++;
    } else {
        // Ring-replace oldest slot once full
        memcpy(fyHiddenBssids[fyHiddenNextSlot], mac, 6);
        fyHiddenNextSlot = (fyHiddenNextSlot + 1) % FY_HIDDEN_DEDUP_SIZE;
    }
}

// Returns pointer to a static buffer holding uptime formatted as +HH:MM:SS.mmm
...
// Returns the match rule name if the SSID matches a Flock signature, else nullptr.
static const char* fyWifiMatchSSID(const char* ssid, size_t len) {
    // Exact: test_flck (dev SSID in production firmware, CVE-2025-59409)
    if (len == 9 && strncasecmp(ssid, "test_flck", 9) == 0) return "ssid_exact";
    // Pattern: Flock-XXXXXX (6 hex digits) per GainSec research
    ...
    // Catch-all: any "flock" substring (case-insensitive)
    if (fyContainsCI(ssid, len, "flock")) return "ssid_substr_flock";
    // Abbrev: any "flck" substring (case-insensitive) — catches test_flck
    // variants and abbreviated deployments without vowels
    if (fyContainsCI(ssid, len, "flck")) return "ssid_substr_flck";
    return nullptr;
}

// Returns the match rule name if the source MAC's OUI matches a Flock-family
// hardware prefix (reuses the BLE detection lists), else nullptr. This catches
// Flock devices regardless of how the installer named the SSID — MAC prefix
// is IEEE-assigned hardware identity.
static const char* fyWifiMatchOUI(const uint8_t* mac) {
    char prefix[9];
    snprintf(prefix, sizeof(prefix), "%02x:%02x:%02x", mac[0], mac[1], mac[2]);
    if (checkFlockMAC(prefix))         return "oui_flock";
    if (checkSoundThinkingMAC(prefix)) return "oui_shotspotter";
    if (checkFlockMfrMAC(prefix))      return "oui_mfr";
    return nullptr;
}
...
static void fyWifiPromiscCallback(void* buf, wifi_promiscuous_pkt_type_t type) {
    ...
    if (rssi < WIFI_SCAN_RSSI_MIN) return;

    fyWifiFramesSeen++;

    const uint8_t* payload = pkt->payload;
    size_t payload_len = pkt->rx_ctrl.sig_len;
    if (payload_len > 4) payload_len -= 4;   // drop FCS
    if (payload_len < 24) return;            // short of MAC header

    uint8_t fc = payload[0];
    uint8_t frame_type = (fc >> 2) & 0x3;
    uint8_t subtype    = (fc >> 4) & 0xF;
    if (frame_type != 0) return;             // must be management

    const uint8_t* src_mac = payload + 10;   // addr2
    ...
    // Hidden-SSID alert: for beacons or probe responses only. Empty SSID in
    // a probe request is a wildcard (normal), not hidden. Alert once per
    // unique BSSID per session so the log stays scannable.
    ...
    if (is_hidden && !fyWifiSeenHidden(src_mac)) {
        ...
        printf("\n[FLOCK-WIFI] [%s] ######## HIDDEN SSID DETECTED ########\n"
               ...
               frame_name, fyWifiChannel, rssi,
               oui_tag ? oui_tag : "unknown",
               fyHiddenCount);
        fyDetectBeep();
    }

    // Check both match families. OUI match wins even without an SSID match
    // because it catches Flocks that renamed the SSID.
    ...
    if (rule) {
        ...
        printf("[FLOCK-WIFI] [%s] MATCH(%s)%s %s "
               ...
               src_mac[0], src_mac[1], src_mac[2],
               src_mac[3], src_mac[4], src_mac[5],
               ssid_str, rssi, fyWifiChannel, fyWifiMatchCount);
        fyDetectBeep();
        return;
    }

    // Anything without SSID data below this point is uninteresting for verbose log
    if (!ssid_data || ssid_len == 0) return;

#ifdef WIFI_SCAN_LOG_ALL
    char ssid_str[33];
    size_t c = ssid_len < 32 ? ssid_len : 32;
    memcpy(ssid_str, ssid_data, c);
    ssid_str[c] = '\0';
    printf("[WIFI] [%s] %s src:%02x:%02x:%02x:%02x:%02x:%02x "
           "ssid:\"%s\" rssi:%d ch:%d\n",
           fyUptimeStr(), frame_name,
           src_mac[0], src_mac[1], src_mac[2],
           src_mac[3], src_mac[4], src_mac[5],
           ssid_str, rssi, fyWifiChannel);
#endif
}

static void fyWifiScanSetup() {
    // Bring up the Arduino WiFi driver in STA mode, but do NOT turn the
    // radio off — the first arg to disconnect() is `wifioff` and must stay
    // false so the MAC stays running for promiscuous mode.
    WiFi.mode(WIFI_STA);
    WiFi.disconnect(false, true);
    delay(100);

    esp_err_t err;
    err = esp_wifi_set_promiscuous(false);  // reset in case framework left it on
    printf("[FLOCK-WIFI] promisc(off): %s\n", esp_err_to_name(err));

    wifi_promiscuous_filter_t filter = {};
    filter.filter_mask = WIFI_PROMIS_FILTER_MASK_MGMT;
    err = esp_wifi_set_promiscuous_filter(&filter);
    printf("[FLOCK-WIFI] filter: %s\n", esp_err_to_name(err));

    err = esp_wifi_set_promiscuous_rx_cb(&fyWifiPromiscCallback);
    printf("[FLOCK-WIFI] rx_cb: %s\n", esp_err_to_name(err));

    err = esp_wifi_set_promiscuous(true);
    printf("[FLOCK-WIFI] promisc(on): %s\n", esp_err_to_name(err));

    err = esp_wifi_set_channel(fyWifiChannel, WIFI_SECOND_CHAN_NONE);
    printf("[FLOCK-WIFI] set_channel(%d): %s\n", fyWifiChannel, esp_err_to_name(err));

    printf("[FLOCK-WIFI] Signatures: Flock-XXXXXX / test_flck / *flock* substring\n");
    printf("[FLOCK-WIFI] RSSI threshold: %d dBm\n", WIFI_SCAN_RSSI_MIN);
    printf("[FLOCK-WIFI] Channel hopping: 1-11 @ 400ms\n");
}

static void fyWifiChannelHop() {
    if (millis() - fyLastChannelHop >= 400) {
        fyWifiChannel++;
        if (fyWifiChannel > 11) fyWifiChannel = 1;
        esp_wifi_set_channel(fyWifiChannel, WIFI_SECOND_CHAN_NONE);
        fyLastChannelHop = millis();
    }
    if (millis() - fyLastWifiStats >= 10000) {
        printf("[FLOCK-WIFI] [%s] frames_seen:%u matches:%u hidden_flagged:%u ch:%d\n",
               fyUptimeStr(), fyWifiFramesSeen, fyWifiMatchCount,
               fyHiddenCount, fyWifiChannel);
        fyLastWifiStats = millis();
    }
}
...
#ifdef FIELD_MODE
    // Field mode: back-to-back scan windows (no idle gap), no WiFi AP.
    // Keeps NimBLE happy (finite durations) while giving BLE all the
    // airtime the classic ESP32 radio can offer.
    fyBleScanDuration = 5;
    fyBleScanInterval = 5100;
#endif
    fyBLEScan->start(fyBleScanDuration, false);
    fyLastBleScan = millis();
#ifdef FIELD_MODE
    printf("[FLOCK-YOU] FIELD MODE: aggressive BLE scan, AP disabled\n");
#else
...
#ifdef FIELD_MODE
    printf("[FLOCK-YOU] Ready - FIELD MODE (BLE-only, no AP)\n\n");
#else
...
void loop() {
    ...
    // Periodic BLE firehose report — every 10s, print how many raw
    // advertisements we heard. Lets you tell in the field whether the
    // radio is hearing anything at all vs. just missing Flock matches.
    if (millis() - fyLastHeardReport >= 10000) {
        uint32_t total = fyTotalHeard;
        uint32_t delta = total - fyHeardAtLastReport;
        printf("[FLOCK-YOU] heard last 10s: %lu  total: %lu  flock-matches: %d\n",
               (unsigned long)delta, (unsigned long)total, fyDetCount);
        fyHeardAtLastReport = total;
        fyLastHeardReport = millis();
    }
